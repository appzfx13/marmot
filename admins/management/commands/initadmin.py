...
from users.models import MarmotUser, TeamMember, MemberRoleChoices, TeamMemberRoleChoices


class Command(BaseCommand):
    help = 'Creates an admin superuser along with MarmotUser and TeamMember admin profiles if they do not exist'
    ...
    def handle(self, *args, **options):
        ...
        # 2. Create or Update MarmotUser entry with ADMIN role
        admin_role = getattr(MemberRoleChoices, 'ADMIN', 'ADMIN')
        
        # Lookup by user link or email
        marmot_user = MarmotUser.objects.filter(user=user).first() or MarmotUser.objects.filter(email=email).first()

        if not marmot_user:
            # Instantiating MarmotUser with user link attached
            marmot_user = MarmotUser(
                user=user,
            ...
            self.stdout.write(self.style.SUCCESS(
                f"MarmotUser created with ADMIN role and username '{marmot_user.username}'."
            ))
        else:
            updated = False
            ...
            if updated:
                marmot_user.save()
                self.stdout.write(self.style.SUCCESS(
                    f"Updated MarmotUser '{marmot_user.username}' with ADMIN role and verified status."
                ))
            else:
                self.stdout.write(self.style.SUCCESS(
                    f"MarmotUser '{marmot_user.username}' already exists and is up to date."
                ))

        # 3. Create or Update TeamMember entry with ADMIN role
        admin_tm_role = getattr(TeamMemberRoleChoices, 'ADMIN', 'ADMIN')
