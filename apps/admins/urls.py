from django.urls import path
from .views import (
    AdminLoginView,
    AdminDashboardView,
    AdminLiveTerminalView,
    AdminSandboxTerminalView,
    AdminTerminalHistoricalDataView,
    AdminTerminalOptionChainAPIView,
    AdminTerminalOperationalDataAPIView,
    AdminTerminalCloseAllPositionsAPIView,
    AdminTerminalCancelOrderAPIView,
    AdminLogoutView,
    AdminTraderListView,
    AdminTraderScrollView,
    AdminTraderCreateView,
    AdminTraderDetailView,
    AdminTraderUpdateView,
    AdminTraderDeleteView,
    AdminTraderPasswordResetView,
    # Trade Exec Config Views
    AdminTradeExecConfigListView,
    AdminTradeExecConfigScrollView,
    AdminTradeExecConfigCreateView,
    AdminTradeExecConfigDetailView,
    AdminTradeExecConfigUpdateView,
    AdminTradeExecConfigDeleteView,
    AdminTradeExecConfigToggleView,
    AdminTradeExecUserAccountInfoView,
    # Postback Views
    PostbackLogListView,
    PostbackLogScrollView,
    PostbackLogDetailView,
    PostbackLogClearView,
    # Broker Master Views
    AdminBrokerMasterListView,
    AdminBrokerMasterCreateModalView,
    AdminBrokerMasterSaveView,
    AdminBrokerMasterUpdateModalView,
    AdminBrokerMasterDeleteModalView,
    AdminBrokerMasterDeleteView,
    AdminTraderBulkDeleteView,
    AdminTradeExecConfigBulkDeleteView,
    PostbackLogBulkDeleteView,
    AdminBrokerMasterBulkDeleteView,
    AdminLiveDashboardView,
    AdminLiveMockDashboardView,
    AdminGatewayEmulatorView,
    AdminGatewaySessionSaveView,
    AdminGatewaySessionListView,
    AdminGatewaySessionDetailView,
    AdminGatewaySessionToggleFavoriteView,
    AdminGatewaySessionDeleteView,
    AdminLiveMockClearSessionView,
    AdminMockBrokerStatsView,
    AdminMockBrokerCalendarView,
    AdminMockBrokerTradesView,
    AdminMockBrokerControlView,
    AdminLiveMacroRibbonView,
    AdminLiveTickAPIView,
    AdminLiveOptionChainPartialView,
    AdminLivePositionsPartialView,
    AdminLiveHoldingsPartialView,
    AdminLiveOrdersPartialView,
    AdminLiveOrderPlaceView,
    AdminLiveOrderCancelView,
    AdminLivePositionSquareOffView,
    AdminSandboxDashboardView,
    AdminSandboxDeploymentsView,
    AdminSandboxCardsPartialView,
    AdminSandboxPositionsPartialView,
    AdminSandboxPositionSquareOffView,
    AdminSandboxOrdersPartialView,
    AdminSandboxOrderCancelView,
    AdminSandboxOrderRcaView,
    AdminSandboxJournalView,
    AdminSandboxJournalStatsView,
    AdminSandboxJournalCalendarView,
    AdminSandboxJournalChartView,
    AdminSandboxJournalOrdersView,
    AdminSandboxAccountCreateModalView,
    AdminSandboxAccountCreateView,
    AdminConnectMockBrokerView,
    AdminSandboxAccountDeleteModalView,
    AdminSandboxAccountDeleteView,
    LiveStrategyEditModalView,
    LiveStrategyEditSaveView,
    AdminAIDashboardView,
    SiteSettingsAdminView,
    SiteSettingsLogoUploadView,
    LiveDataFeedView,
    EndpointsDirectoryView,
    FyersAuthCallbackView,
    LiveStrategyToggleModalView,
    LiveStrategyToggleView,
    LiveStrategyDeleteView,
    MasterLiveExecutionToggleModalView,
    MasterLiveExecutionToggleView,
    # Trading Journal & Analytics Views
    AdminJournalView,
    AdminJournalStatsView,
    AdminJournalCalendarView,
    AdminJournalChartView,
    AdminJournalTradesView,
    AdminStrategyCheckView,
)

from apps.market.views import MarketBackupListView, MarketBackupChartView, MarketBackupBulkDeleteView
from apps.backtest.views import BacktestDashboardView, BacktestBulkDeleteView

app_name = 'admins' 

urlpatterns = [
    # Auth & Dashboards
    path('login/', AdminLoginView.as_view(), name='admin-login'),
    path('dashboard/', AdminDashboardView.as_view(), name='admin-dashboard'),
    path('dashboard/live/', AdminLiveDashboardView.as_view(), name='admin-live-dashboard'),
    path('dashboard/live-mock/', AdminLiveMockDashboardView.as_view(), name='admin-live-mock-dashboard'),
    path('dashboard/gateway-emulator/', AdminGatewayEmulatorView.as_view(), name='admin-gateway-emulator'),
    path('dashboard/gateway-emulator/sessions/', AdminGatewaySessionListView.as_view(), name='admin-gateway-session-list'),
    path('dashboard/gateway-emulator/sessions/save/', AdminGatewaySessionSaveView.as_view(), name='admin-gateway-session-save'),
    path('dashboard/gateway-emulator/sessions/<int:pk>/', AdminGatewaySessionDetailView.as_view(), name='admin-gateway-session-detail'),
    path('dashboard/gateway-emulator/sessions/<int:pk>/toggle-favorite/', AdminGatewaySessionToggleFavoriteView.as_view(), name='admin-gateway-session-toggle-favorite'),
    path('dashboard/gateway-emulator/sessions/<int:pk>/delete/', AdminGatewaySessionDeleteView.as_view(), name='admin-gateway-session-delete'),
    path('dashboard/live-mock/stats/', AdminMockBrokerStatsView.as_view(), name='admin-mock-broker-stats'),
    path('dashboard/live-mock/calendar/', AdminMockBrokerCalendarView.as_view(), name='admin-mock-broker-calendar'),
    path('dashboard/live-mock/trades/', AdminMockBrokerTradesView.as_view(), name='admin-mock-broker-trades'),
    path('dashboard/live-mock/control/', AdminMockBrokerControlView.as_view(), name='admin-mock-broker-control'),
    path('dashboard/live-mock/clear-session/', AdminLiveMockClearSessionView.as_view(), name='admin-live-mock-clear-session'),
    path('dashboard/live/macro-ribbon/', AdminLiveMacroRibbonView.as_view(), name='admin-live-macro-ribbon'),
    path('dashboard/live/tick/', AdminLiveTickAPIView.as_view(), name='admin-live-tick'),
    path('dashboard/live/option-chain/', AdminLiveOptionChainPartialView.as_view(), name='admin-live-option-chain'),
    path('dashboard/live/positions/', AdminLivePositionsPartialView.as_view(), name='admin-live-positions-partial'),
    path('dashboard/live/holdings/', AdminLiveHoldingsPartialView.as_view(), name='admin-live-holdings-partial'),
    path('dashboard/live/orders/', AdminLiveOrdersPartialView.as_view(), name='admin-live-orders-partial'),
    path('dashboard/live/orders/place/', AdminLiveOrderPlaceView.as_view(), name='admin-live-order-place'),
    path('dashboard/live/orders/<str:order_id>/cancel/', AdminLiveOrderCancelView.as_view(), name='admin-live-order-cancel'),
    path('dashboard/live/positions/square-off/', AdminLivePositionSquareOffView.as_view(), name='admin-live-position-square-off'),
    path('dashboard/sandbox/', AdminSandboxDashboardView.as_view(), name='admin-sandbox-dashboard'),
    path('dashboard/sandbox/cards/', AdminSandboxCardsPartialView.as_view(), name='admin-sandbox-cards-partial'),
    path('dashboard/sandbox/deployments/', AdminSandboxDeploymentsView.as_view(), name='admin-sandbox-deployments'),
    path('dashboard/sandbox/positions/', AdminSandboxPositionsPartialView.as_view(), name='admin-sandbox-positions-partial'),
    path('dashboard/sandbox/positions/square-off/', AdminSandboxPositionSquareOffView.as_view(), name='admin-sandbox-position-square-off'),
    path('dashboard/sandbox/orders/', AdminSandboxOrdersPartialView.as_view(), name='admin-sandbox-orders-partial'),
    path('dashboard/sandbox/orders/<str:order_id>/cancel/', AdminSandboxOrderCancelView.as_view(), name='admin-sandbox-order-cancel'),
    path('dashboard/sandbox/orders/<str:order_id>/rca/', AdminSandboxOrderRcaView.as_view(), name='admin-sandbox-order-rca'),
    path('dashboard/sandbox/journal/', AdminSandboxJournalView.as_view(), name='admin-sandbox-journal'),
    path('dashboard/sandbox/journal/stats/', AdminSandboxJournalStatsView.as_view(), name='admin-sandbox-journal-stats'),
    path('dashboard/sandbox/journal/calendar/', AdminSandboxJournalCalendarView.as_view(), name='admin-sandbox-journal-calendar'),
    path('dashboard/sandbox/journal/chart/', AdminSandboxJournalChartView.as_view(), name='admin-sandbox-journal-chart'),
    path('dashboard/sandbox/journal/orders/', AdminSandboxJournalOrdersView.as_view(), name='admin-sandbox-journal-orders'),
    path('dashboard/sandbox/accounts/create-modal/', AdminSandboxAccountCreateModalView.as_view(), name='admin-sandbox-account-create-modal'),
    path('dashboard/sandbox/accounts/create/', AdminSandboxAccountCreateView.as_view(), name='admin-sandbox-account-create'),
    path('dashboard/sandbox/accounts/<int:pk>/delete-modal/', AdminSandboxAccountDeleteModalView.as_view(), name='admin-sandbox-account-delete-modal'),
    path('dashboard/sandbox/accounts/<int:pk>/delete/', AdminSandboxAccountDeleteView.as_view(), name='admin-sandbox-account-delete'),
    path('dashboard/sandbox/account-create-modal/', AdminSandboxAccountCreateModalView.as_view(), name='sandbox-account-create-modal'),
    path('dashboard/sandbox/account-create/', AdminSandboxAccountCreateView.as_view(), name='sandbox-account-create'),
    path('dashboard/mock/connect/', AdminConnectMockBrokerView.as_view(), name='admin-connect-mock-broker'),
    path('dashboard/sandbox/account-delete-modal/<int:pk>/', AdminSandboxAccountDeleteModalView.as_view(), name='sandbox-account-delete-modal'),
    path('dashboard/sandbox/account-delete/<int:pk>/', AdminSandboxAccountDeleteView.as_view(), name='sandbox-account-delete'),
    path('dashboard/ai/', AdminAIDashboardView.as_view(), name='admin-ai-dashboard'),
    path('live/terminal/', AdminLiveTerminalView.as_view(), name='admin-live-terminal'),
    path('terminal/live/', AdminLiveTerminalView.as_view(), name='admin-terminal-live-alias'),
    path('sandbox/terminal/', AdminSandboxTerminalView.as_view(), name='admin-sandbox-terminal'),
    path('terminal/sandbox/', AdminSandboxTerminalView.as_view(), name='admin-terminal-sandbox-alias'),
    path('gateway/emulator/', AdminGatewayEmulatorView.as_view(), name='admin-gateway-emulator-alias'),
    path('brokers/', AdminBrokerMasterListView.as_view(), name='admin-brokers-alias'),
    path('site-settings/', SiteSettingsAdminView.as_view(), name='admin-site-settings-alias'),
    path('terminal/historical-data/', AdminTerminalHistoricalDataView.as_view(), name='admin-terminal-historical-data'),
    path('terminal/option-chain/', AdminTerminalOptionChainAPIView.as_view(), name='admin-terminal-option-chain'),
    path('terminal/operational-data/', AdminTerminalOperationalDataAPIView.as_view(), name='admin-terminal-operational-data'),
    path('terminal/positions/close-all/', AdminTerminalCloseAllPositionsAPIView.as_view(), name='admin-terminal-close-all-positions'),
    path('terminal/orders/cancel/', AdminTerminalCancelOrderAPIView.as_view(), name='admin-terminal-cancel-order'),
    path('logout/', AdminLogoutView.as_view(), name='admin-logout'),

    # Trading Journal & Analytics Routes
    path('journal/', AdminJournalView.as_view(), name='admin-journal'),
    path('journal/stats/', AdminJournalStatsView.as_view(), name='admin-journal-stats'),
    path('journal/calendar/', AdminJournalCalendarView.as_view(), name='admin-journal-calendar'),
    path('journal/chart/', AdminJournalChartView.as_view(), name='admin-journal-chart'),
    path('journal/trades/', AdminJournalTradesView.as_view(), name='admin-journal-trades'),
    path('strategy-check/', AdminStrategyCheckView.as_view(), name='admin-strategy-check'),

    # Control Routes for Market Backup & Backtest
    path('market-backup/', MarketBackupListView.as_view(), name='market_backup_list'),
    path('market-backup/bulk-delete/', MarketBackupBulkDeleteView.as_view(), name='market_backup_bulk_delete'),
    path('market-backup/<int:pk>/chart/', MarketBackupChartView.as_view(), name='market_backup_chart'),
    path('backtest/', BacktestDashboardView.as_view(), name='backtest_dashboard'),
    path('backtest/bulk-delete/', BacktestBulkDeleteView.as_view(), name='backtest_bulk_delete'),

    # Traders Management (Aligned with template route names)
    path('traders/', AdminTraderListView.as_view(), name='trader_list'),
    path('traders/scroll/', AdminTraderScrollView.as_view(), name='trader_scroll'),
    path('traders/bulk-delete/', AdminTraderBulkDeleteView.as_view(), name='trader_bulk_delete'),
    path('traders/create/', AdminTraderCreateView.as_view(), name='trader_create'),
    path('traders/<int:pk>/', AdminTraderDetailView.as_view(), name='trader_detail'),
    path('traders/<int:pk>/edit/', AdminTraderUpdateView.as_view(), name='trader_edit'),
    path('traders/<int:pk>/delete/', AdminTraderDeleteView.as_view(), name='trader_delete'),
    path('traders/<int:pk>/password-reset/', AdminTraderPasswordResetView.as_view(), name='trader_password_reset'),

    # Postback & Webhook Audit Logs
    path('postbacks/', PostbackLogListView.as_view(), name='postback_list'),
    path('postbacks/scroll/', PostbackLogScrollView.as_view(), name='postback_scroll'),
    path('postbacks/clear/', PostbackLogClearView.as_view(), name='postback_clear'),
    path('postbacks/bulk-delete/', PostbackLogBulkDeleteView.as_view(), name='postback_bulk_delete'),
    path('postbacks/<int:pk>/', PostbackLogDetailView.as_view(), name='postback_detail'),

    # Trade Execution Configurations
    path('trade-configs/', AdminTradeExecConfigListView.as_view(), name='trade_exec_config_list'),
    path('trade-configs/scroll/', AdminTradeExecConfigScrollView.as_view(), name='trade_exec_config_scroll'),
    path('trade-configs/bulk-delete/', AdminTradeExecConfigBulkDeleteView.as_view(), name='trade_exec_config_bulk_delete'),
    path('trade-configs/create/', AdminTradeExecConfigCreateView.as_view(), name='trade_exec_config_create'),
    path('trade-configs/user-account-info/', AdminTradeExecUserAccountInfoView.as_view(), name='trade_exec_user_account_info'),
    path('trade-configs/<int:pk>/', AdminTradeExecConfigDetailView.as_view(), name='trade_exec_config_detail'),
    path('trade-configs/<int:pk>/edit/', AdminTradeExecConfigUpdateView.as_view(), name='trade_exec_config_edit'),
    path('trade-configs/<int:pk>/delete/', AdminTradeExecConfigDeleteView.as_view(), name='trade_exec_config_delete'),
    path('trade-configs/<int:pk>/toggle/', AdminTradeExecConfigToggleView.as_view(), name='trade_exec_config_toggle'),

    # Master Brokers Management
    path('masters/brokers/', AdminBrokerMasterListView.as_view(), name='broker-master-list'),
    path('masters/brokers/bulk-delete/', AdminBrokerMasterBulkDeleteView.as_view(), name='broker-master-bulk-delete'),
    path('masters/brokers/create-modal/', AdminBrokerMasterCreateModalView.as_view(), name='broker-master-create-modal'),
    path('masters/brokers/create/', AdminBrokerMasterSaveView.as_view(), name='broker-master-create'),
    path('masters/brokers/<int:pk>/edit-modal/', AdminBrokerMasterUpdateModalView.as_view(), name='broker-master-edit-modal'),
    path('masters/brokers/<int:pk>/edit/', AdminBrokerMasterSaveView.as_view(), name='broker-master-edit'),
    path('masters/brokers/<int:pk>/delete-modal/', AdminBrokerMasterDeleteModalView.as_view(), name='broker-master-delete-modal'),
    path('masters/brokers/<int:pk>/delete/', AdminBrokerMasterDeleteView.as_view(), name='broker-master-delete'),

    # Site Settings & Live Feed Management
    path('settings/', SiteSettingsAdminView.as_view(), name='site-settings'),
    path('settings/upload-logo/<str:field_name>/', SiteSettingsLogoUploadView.as_view(), name='site-settings-logo-upload'),
    path('live-data-feed/', LiveDataFeedView.as_view(), name='live-data-feed'),
    path('endpoints/', EndpointsDirectoryView.as_view(), name='endpoints-directory'),
    path('fyers/callback/', FyersAuthCallbackView.as_view(), name='fyers-oauth-callback'),
    path('fyers/callback', FyersAuthCallbackView.as_view()),

    # Live Strategy Execution Controls
    path('live-strategies/<int:pk>/toggle-modal/', LiveStrategyToggleModalView.as_view(), name='live-strategy-toggle-modal'),
    path('live-strategies/<int:pk>/toggle/', LiveStrategyToggleView.as_view(), name='live-strategy-toggle'),
    path('live-strategies/<int:pk>/edit-modal/', LiveStrategyEditModalView.as_view(), name='live-strategy-edit-modal'),
    path('live-strategies/<int:pk>/edit-save/', LiveStrategyEditSaveView.as_view(), name='live-strategy-edit-save'),
    path('live-strategies/<int:pk>/delete/', LiveStrategyDeleteView.as_view(), name='live-strategy-delete'),
    path('live-strategies/master-switch-modal/', MasterLiveExecutionToggleModalView.as_view(), name='master-live-execution-modal'),
    path('live-strategies/master-switch-toggle/', MasterLiveExecutionToggleView.as_view(), name='master-live-execution-toggle'),
]