...
from users.choices import BrokerChoices, MemberRoleChoices, PLStatusChoices, TeamMemberRoleChoices
...
class User(AbstractUser):
    # Optional custom user fields can go here
    pass


class MarmotUser(models.Model):
    name = models.CharField(max_length=100)
    
    # Username (blank=True allows auto-generation before validation)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and not self.username:
            ...
            while MarmotUser.objects.filter(username=new_username).exists():
                new_username = self.generate_unique_username()
